        ...
        if pairs_bought != 'no pairs':
            ...
            for pair in pairs_bought:
                ...
                four_h_data = polo.returnChartData(
                    pair['name'], period=CANDLE_4H_PERIOD, start=last_bought_date - CANDLE_4H_PERIOD)[:-1]
                candles_4h_data = [
                ...
                first_candle_condition = True if len(candles_4h_data) == 1 and not is_dodge(
                    candles_4h_data[0]) and is_sword(candles_4h_data[0]) else False
                second_candle_condition = False
                third_candle_condition = True if len(candles_4h_data) == 3 and not is_green(
                    candles_4h_data[-1]) and is_green(candles_4h_data[-2]) else False
                if len(candles_4h_data) == 2 and ((is_green(candles_4h_data[0]) and not is_green(
                        candles_4h_data[1]) and ((is_simple(candles_4h_data[0]) and is_hummer(candles_4h_data[1])) or (
                            is_simple(candles_4h_data[0]) and is_sword(candles_4h_data[1])) or (
                            is_hummer(candles_4h_data[0]) and is_simple(candles_4h_data[1])) or (
                            is_sword(candles_4h_data[0]) and is_simple(candles_4h_data[1])) or (
                            is_dodge(candles_4h_data[0]) and is_simple(candles_4h_data[1])))) or (
                                not is_green(candles_4h_data[0]) and not is_green(candles_4h_data[1]) and (
                                        (is_simple(candles_4h_data[0]) and is_simple(candles_4h_data[1])) or (
                                                is_hummer(candles_4h_data[0]) and is_simple(candles_4h_data[1])) or (
                                            is_sword(candles_4h_data[0]) and is_simple(candles_4h_data[1]))))):
                ...
                if altcoin_amount > 0 and \
                        (time.time() - last_bought_date >= CANDLE_PERIOD or sell_price < STOP_LOSS * pair['price']
                         or sell_price > TAKE_PROFIT * pair['price'] or first_candle_condition
                         or second_candle_condition or third_candle_condition):
